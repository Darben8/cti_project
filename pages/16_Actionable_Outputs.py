...
def load_and_map_data():
    """Load IOC data from multiple sources."""
    ...
    # 1. Load All IOCs
    try:
        all_iocs_path = data_dir / "all_iocs.csv"
        if all_iocs_path.exists():
            df_all = pd.read_csv(all_iocs_path)
            if not df_all.empty:
                df_all['source'] = df_all.get('source', 'Combined')
                dfs["All IOCs"] = df_all
    except Exception as e:
        st.sidebar.error(f"Error loading all_iocs.csv: {e}")
    
    # 2. Load Combined IOCs
    try:
        combined_path = data_dir / "combined_iocs.csv"
        if combined_path.exists():
            df_combined = pd.read_csv(combined_path)
            if not df_combined.empty:
                df_combined['source'] = df_combined.get('source', 'Combined')
                dfs["Combined IOCs"] = df_combined
    except Exception as e:
        st.sidebar.error(f"Error loading combined_iocs.csv: {e}")
    
    # 3. Load ThreatFox Data
    try:
        threatfox_path = data_dir / "filtered_iocs_threatfox.csv"
        if threatfox_path.exists():
    ...
    # 4. Load Phishing Data
    try:
        phishing_path = data_dir / "verified_online_banking_finance.csv"
        if phishing_path.exists():
            df_phishing = pd.read_csv(phishing_path)
            if not df_phishing.empty:
                df_phishing['source'] = 'Phishtank URLs'
                # Standardize column names if needed
                if 'url' in df_phishing.columns and 'indicator' not in df_phishing.columns:
                    df_phishing = df_phishing.rename(columns={'url': 'indicator'})
                dfs["Phishtank URLs"] = df_phishing
    except Exception as e:
        st.sidebar.error(f"Error loading verified_online_banking_finance.csv: {e}")
...
@st.cache_data
def get_ioc_datasets():
    """Load and cache IOC datasets."""
    return load_and_map_data()

ioc_datasets = get_ioc_datasets()

if not ioc_datasets:
    st.error("❌ No data files found. Please ensure CSV files are in the data/ folder.")
    st.stop()
...
with tab1:
    ...
    st.markdown("### Course-of-Action Recommendations")
    ...
    st.markdown("### Indicators with COA Recommendations")
...
with tab2:
    ...
    with col3:
        ...
        st.markdown("""
        **Best for:**
        - Standards-base_folderd sharing
...
with tab4:
    ...
    **1. Threat Categorization**
    - Automatically categorizes indicators base_folderd on type and tags
    ...
    **Step 3: Execute**
    ...
    - Adjust base_folderd on results
    
    **Step 4: Track & Improve**
    - Monitor effectiveness
    - Update TTL as needed
    - Refine COA database_folder
