import streamlit as st

st.title(" Team ")

st.markdown("""
**Jemima Lumbala**
Milestone 1 Role: Alignment, streamlit app page navigation, threat intel buy-in page
Milestone 2 Role: Data Source Identification and Justification
Signature: Jemima Lumbala (03/25/2026)

**Abena Darko**
Milestone 1 Role: Threat Intelligence Research & API integration, dashboard development, and write-up
Milestone 2 Role: Ethics and Data Governance, Security-Aware Development Practices, App review
Signature: Abena Darko (03/25/2026)

**Shani Nanje**
Milestone 1 Role: Threat Intelligence Use Case Development
Milestone 2 Role: Data Source Identification and Justification
Signature: Shani Nanje (03/25/2026)

**Otis Service**
Milestone 1 Role: Project Manager, Industry Research, Streamlit app development, and write-up
Milestone 2 Role:
Signature: Otis Service (03/25/2026)
            
**Jiwon Chang**
Milestone 1 Role: Intelligence buy in research
Milestone 2 Role: Data explorer page development
Signature: Jiwon Chang (04/07/2026)
            
**Supradipta Panta**
Milestone 1 Role: Code review
Milestone 2 Role: Data Collection and Summary
Signature: Supradipta Panta (04/07/2026)
""")
